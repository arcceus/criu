...
static int utsns_broker_enter_creds(void)
{
	if ((geteuid() != 0 && setuid(0)) || (getegid() != 0 && setgid(0))) {
		pr_perror("utsns broker: set uid/gid 0 in target userns");
		return -1;
	}
	...
	if (setgroups(0, NULL)) {
		if (errno == EPERM) {
			...
			if (nr_groups == 0 || (nr_groups == 1 && groups[0] == 0)) {
				pr_info("utsns broker: setgroups denied, groups already safe\n");
				return 0;
			}

			if (in_noninitial_userns()) {
				pr_info("utsns broker: tolerate setgroups denial in user namespace\n");
				return 0;
			}
		}
		pr_perror("utsns broker: setgroups in target userns");
		return -1;
	}
