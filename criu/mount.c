...
static struct mount_info *find_fsroot_mount_for(struct mount_info *bm)
{
	...
	/*
	 * Rootless OCI runtimes bind individual devtmpfs nodes
	 * (/dev/urandom, /dev/null, …) from the same host superblock.
	 * search_bindmounts() groups them by s_dev only, so mnt_bind
	 * does not contain a mount with root "/". Walk the parent chain
	 * to find the real FS root (e.g. tmpfs on /dev with root "/").
	 */
...
static int get_clean_fd(struct mount_info *mi)
{
	...
	if (criu_umount2_at(mnt_path, MNT_DETACH)) {
...
again:
	...
	while (m != mi) {
		if (criu_umount2_at(m->ns_mountpoint, MNT_DETACH)) {
...
next:
	...
	if (ovm) {
		...
		if (criu_umount2_at(ovm->ns_mountpoint, MNT_DETACH)) {
...
int criu_umount2_at(const char *target, int flags)
{
	/*
	 ...
	 * in-process. Mount still uses the broker for container userns creds.
	 */
 ...
 *
 * Host-backed binds in rootless restore (resolv.conf, -v volumes, mount-yard
 * binds) inherit locked flags from the bind source; remount is a no-op at best
 * and EPERM at worst. Skip it when after_bind is true.
 *
...
static int remount_bind_flags(struct mount_info *mi, unsigned long mflags, bool after_bind)
{
	...
	if ((opts.unprivileged || in_noninitial_userns()) && opts.mode == CR_RESTORE &&
		...
		pr_warn("mnt: re-mount EPERM at %s in rootless restore, per-mount flags dropped (nosuid,noexec,nodev,etc.)\n",
			service_mountpoint(mi));
		return 0;
	}
...
{
	if (criu_umount2_at(mnt_clean_path, MNT_DETACH)) {
...
static int do_umount_one(struct mount_info *mi)
{
	...
	if (criu_umount2_at(service_mountpoint(mi), 0)) {
...
int cr_pivot_root(char *root)
{
	...
	if (criu_umount2_at(put_root, MNT_DETACH)) {
...
static int __depopulate_roots_yard(void)
{
	 ...
	 */
	if (criu_umount2_at(mnt_roots, MNT_DETACH)) {
...
int depopulate_roots_yard(int mntns_fd, bool only_ghosts)
{
	...
	old_cwd = open(".", O_PATH);
	if (old_cwd < 0) {
		if (userns_join_ns_requested() && (errno == EACCES || errno == EPERM)) {
			pr_info("Unable to open cwd, using / for joined external userns mount cleanup\n");
			old_cwd = open("/", O_PATH);
			if (old_cwd >= 0)
				goto cwd_saved;
		}
...
cwd_saved:
	...
	if (setns(mntns_fd, CLONE_NEWNS) < 0) {
		if (userns_join_ns_requested() && (errno == EACCES || errno == EPERM)) {
			pr_info("Unable to switch to mntns for joined external userns cleanup, leaving roots yard to runtime namespace teardown\n");
			close(old_ns);
			if (fchdir(old_cwd))
